...
**State:** Results-spine draft complete and stress-tested. A 5-referee review board ran and its findings were actioned: transport now positioned against Bresnan & Ford 2010 and Röthlisberger et al. 2017, bootstrap test-set CIs added (transport complete-case 0.308 [0.277, 0.337]; native complete-case 0.202 [0.154, 0.254], non-overlapping), projectibility cashed out (Boyd "for a purpose"), grammaticality construct cited (Pullum, Nefdt) instead of stipulated, structural-blindness named against competence/performance (Chomsky 1965, Miller 1963). Full numbers audit: 60/60 PASS against source. Sampling-scope audit narrows the transport claim to the six-verb high-frequency core and labels the BNC2014 headline metrics as complete-case rows; all-row imputation sensitivity is reproducible in `analysis/07_sampling_tilt_sensitivity.R`. DAIS reuse is now unblocked by Robert Hawkins's 2026-06-25 permission email and the upstream CC BY 4.0 licence added 2026-06-24. `analysis/08_dais_acceptability_bridge.R` adds the compact acceptability bridge for the 150 shared-verb DAIS items without committing raw DAIS data. The Gelman/Grieve upgrade adds explicit estimands, a DAIS calibration figure and participant-level mixed check, and a BNC2014 metadata-scope audit that keeps the transport result from being overread as a clean population or variety estimate.
**Next action:** Optional later: re-run the board on the revised draft, or do a final submission polish. The DAIS bridge and BNC2014 scope audit are implemented and verified.
