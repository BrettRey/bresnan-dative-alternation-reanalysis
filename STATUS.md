...
**State:** Major-revision response in progress. The old transported-vs-native BNC2014 holdout comparison is superseded. `analysis/10_bnc2014_paired_transport_cv.R` now compares source-trained and BNC2014-native models on the same 1,621 complete-case rows, with repeated 10-fold row-level native out-of-fold predictions. Grouped speaker/conversation folds cannot be reconstructed from the released BNC2014 final CSV: the support scripts show `main_speaker_id` existed upstream but was dropped before public release. Current paired result: source full 0.308 log loss / 0.906 AUC; source non-verb 0.296 / 0.899; native full 0.227 / 0.936. `analysis/09_bnc2014_metadata_scope.R` now writes harmonization, missingness, and reduced all-row checks. `analysis/08_dais_acceptability_bridge.R` now fits the DAIS participant model with fixed verb effects and crossed participant/item intercepts; the production-score coefficient is 0.017 (SE 0.076), supporting shared conditioning plus channel separation. The manuscript has been revised around same-row evaluation, missingness, item-level DAIS structure, and graded evidence for licensing rather than ontically graded grammaticality.
**Next action:** Run full verification: DAIS script, BNC metadata script, paired-CV script, figure generation, style linter, `make`, log scan, and `git diff --check`.
