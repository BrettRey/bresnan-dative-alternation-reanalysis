...
BNC2014 tokens, so a majority-class baseline already scores near 0.82, and the
transported model reaches only about 0.86. The gain that matters shows up in
discrimination and calibration, the AUC of 0.906 and the lower log loss, not in
...
holdout split,
the native core model reaches 0.202 log loss and 0.960 AUC. The gap is not
small: the transported model's log loss is about half again the native model's.
The transported model has learned a profile that's genuinely useful in the new
corpus, but the BNC2014 data also contains local structure that the
\texttt{languageR} fit doesn't capture.
