...
baseline, and its AUC is 0.906. A percentile bootstrap over the BNC2014 test
rows (2,000 resamples) puts the log loss in [0.277, 0.337] and the AUC in
[0.890, 0.922], so the result isn't an artefact of one lucky split. When the
BNC2014 outcome is scrambled, the same predictions collapse to 0.885 log loss
and 0.515 AUC.
...
just split noise. The transported model has learned a profile that's genuinely
useful in the new corpus, but the BNC2014 data also contains local structure
that the \texttt{languageR} fit doesn't capture.
